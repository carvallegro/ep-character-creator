...
const Background = ({ background = {} }) => (
  <Card title={background.name}>
    ...
    <div>
      {map(t => (
        <TooltipDefinition key={t.atomUid} tooltipText={t.description}>
          {t.name}
        </TooltipDefinition>
      ))(background.traits)}
    </div>
    ...
    <div>{map(bm => bm.name)(background.bonusMalus).join(', ')}</div>

    <FormLabel>Limitations</FormLabel>
    <div>{map(bm => bm)(background.limitations).join(', ')}</div>
...
const BackgroundEnhanced = withColumn({ xs: 4 })(
  connect(mapStateToProps)(Background)
)

const BackgroundLayout = () => (
  <React.Fragment>
    <ChooseBackgroundEnhanced />
    <BackgroundEnhanced />
  </React.Fragment>
)
